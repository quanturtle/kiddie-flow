...
export function TextNode({ id, data }: NodeProps) {
  ...
  return (
    <div 
      className={`rounded-lg border-4 p-4 min-w-[500px] relative shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] ${typeColors[data.type]}`}
      style={{ transform: 'translate(-50%, -50%)' }}
    >
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center gap-2">
          <span className={`text-xs uppercase ${data.type === 'result' ? 'bg-white text-black' : 'bg-black text-white'} px-1 py-0.5 rounded`}>
            {data.type}
          </span>
          <h3 className="font-bold">{data.title || `Node ${id}`}</h3>
        </div>
